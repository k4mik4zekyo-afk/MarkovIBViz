"""
readme_generator.py — Generate README.md from Agent.md.

Per Agent.md:
- README.md is treated as a derived artifact, not manually maintained.
- Must include: project overview, data requirements, how to run,
  description of outputs, testing philosophy, clear statement that
  results are empirical not predictive.
"""

import os

AGENT_MD = os.path.join(os.path.dirname(__file__), "Agent.md")
README_MD = os.path.join(os.path.dirname(__file__), "README.md")


def generate_readme(agent_md_path=None, output_path=None):
    """Generate README.md programmatically from Agent.md."""
    if agent_md_path is None:
        agent_md_path = AGENT_MD
    if output_path is None:
        output_path = README_MD

    readme = """# Value Discovery State Agent
